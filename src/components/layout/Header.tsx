      ...
      <div className="flex items-center gap-2 shrink-0">
        ...
        <div className="relative" ref={themeRef}>
          <button
            ...
            className={`w-10 h-10 rounded-full flex items-center justify-center transition-all ${themeOpen
              ? 'bg-(--bg-elevated) text-(--text-accent)'
            ...
            <div className="absolute right-0 top-full mt-2 w-40 bg-(--bg-surface) border border-(--border-subtle) rounded-xl shadow-xl overflow-hidden z-50">
              {THEME_OPTIONS.map(({ id, icon: Icon, label }) => (
                <button
                  ...
                  className={`w-full flex items-center gap-3 px-4 py-2.5 text-sm transition-all hover:bg-(--bg-elevated) ${theme === id
                    ? 'text-(--text-accent) bg-(--badge-bg) font-medium'
                    : 'text-(--text-secondary)'
                    }`}>
                  <Icon size={15} />
                  <span>{label}</span>
